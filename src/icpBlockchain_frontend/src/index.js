import { icpBlockchain_backend } from "../../declarations/icpBlockchain_backend";
...
appointForm.addEventListener("submit", async (e) => {
  ...
  try {
    const getId = await icpBlockchain_backend.addTodo(obj.textarea);
...
window.addEventListener("DOMContentLoaded", async () => {
  try {
    const getAll = await icpBlockchain_backend.getAllTodo();
...
async function deleteTodo(userId) {
  try {
    ...
    await icpBlockchain_backend.deleteTodo(userId);
